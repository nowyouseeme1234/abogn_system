...
const Overview = () => {
  ...
  const items = itemList.map(itemList =>
    <Link to={'/dashboard/'+ itemList.title.toLowerCase()} key={itemList.id} className=' size-52 bg-gray-300 rounded-3xl'>
  ...
  return (
    <div className='flex bg-secondary h-screen rounded-[5rem] w-auto'>
        ...
        {/* main part */}
        <div className='flex justify-around gap-16 items-center mt-20'>
          {items}
        </div>
    </div>
